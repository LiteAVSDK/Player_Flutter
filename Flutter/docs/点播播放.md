...
播放器高级版  SDK 支持播放商业级 DRM 加密视频，目前支持 WideVine 和 Fairplay 两种 DRM 方案。更多的商业级 DRM 信息，请参考 [商业级 DRM 综述](https://cloud.tencent.com/document/product/266/79724)。

```dart
// 通过 startPlayDrm 接口播放
// @param licenseUrl 解密的key url
// @param videoUrl 待播放视频的Url地址
// @param certificateUrl 证书提供商url
_controller.startPlayDrm(TXPlayerDrmBuilder("${playUrl}","${licenseUrl}","${deviceCertificateUrl}"));
```
...
当提前知道播放的媒资类型时，可以通过配置`FTXVodPlayConfig#setMediaType`减少播放器SDK内部播放类型探测，提升启播速度。
