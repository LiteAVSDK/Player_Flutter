...
//        Log.d("AndroidLog", "textureId :" + mSurfaceTextureEntry.id());
    ...
    void setPlayConfig(Map config) {
        if (mVodPlayer != null) {
            TXVodPlayConfig playConfig = new TXVodPlayConfig();

            int connectRetryCount = (int) config.get("connectRetryCount");
            if (connectRetryCount > 0) {
                playConfig.setConnectRetryCount(connectRetryCount);
            }

            int connectRetryInterval = (int) config.get("connectRetryInterval");
            if (connectRetryInterval > 0) {
                playConfig.setConnectRetryInterval(connectRetryInterval);
            }

            int timeout = (int) config.get("timeout");
            if (timeout > 0) {
                playConfig.setTimeout(timeout);
            }

            String cacheFolderPath = (String) config.get("cacheFolderPath");
            if (!TextUtils.isEmpty(cacheFolderPath)) {
                playConfig.setCacheFolderPath(cacheFolderPath);
            } else {
                playConfig.setCacheFolderPath(null);
            }

            int maxCacheItems = (int) config.get("maxCacheItems");
            if (maxCacheItems > 0) {
                playConfig.setMaxCacheItems(maxCacheItems);
            }

            playConfig.setPlayerType((int) config.get("playerType"));
            ...
            int progressInterval = (int) config.get("progressInterval");
            if (progressInterval > 0) {
                playConfig.setProgressInterval(progressInterval);
            }

            int maxBufferSize = (int) config.get("maxBufferSize");
            if (maxBufferSize > 0) {
                playConfig.setMaxBufferSize(maxBufferSize);
            }

            int maxPreloadSize = (int) config.get("maxPreloadSize");
            if (maxPreloadSize > 0) {
                playConfig.setMaxPreloadSize(maxPreloadSize);
            }

            int firstStartPlayBufferTime = (int) config.get("firstStartPlayBufferTime");
            playConfig.setFirstStartPlayBufferTime(firstStartPlayBufferTime);

            int nextStartPlayBufferTime = (int) config.get("nextStartPlayBufferTime");
            ...
            playConfig.mEnableRenderProcess = (boolean) config.get("enableRenderProcess");
